...
import system_microservice_share_documents.CDCUpdateUserDataCache.config.ElasticsearchConfig;

import java.util.HashMap;
import java.util.Map;

public class UserElasticsearchSink extends RichSinkFunction<Map<String, Object>> {

    private static final Logger log = LoggerFactory.getLogger(UserElasticsearchSink.class);
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        // ensure index exists
        GetIndexRequest existsRequest = new GetIndexRequest("users");
        boolean exists = client.indices().exists(existsRequest, RequestOptions.DEFAULT);

        if (!exists) {
            log.info("Index 'users' does not exist → creating...");
            CreateIndexRequest request = new CreateIndexRequest("users");
            client.indices().create(request, RequestOptions.DEFAULT);
        } else {
            log.info("Index 'users' already exists. Skip creation.");
        }
    }

    @Override
    public void invoke(Map<String, Object> userData, Context context) {
        if (userData == null) return;

        Object idObj = userData.get("id");
        if (idObj == null) {
            log.error("Missing ID field. Skipping record: {}", userData);
            return;
        }

        String id = idObj.toString();
        String operation = String.valueOf(userData.get("_operation"));

        switch (operation) {
            case "CREATE":
            case "UPDATE":
            case "SNAPSHOT":
                indexDocument(id, userData);
                break;
    ...
    private void indexDocument(String id, Map<String, Object> source) {
        try {
            ...
            Object metaVal = safeSource.remove("profile"); // REMOVE FIRST
            ...
            safeSource.put("profile", metaMap);
            ...
            log.error("[ES] Error preparing user {}: {}", id, e.getMessage(), e);
        }
    }
